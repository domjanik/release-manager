...
async function getVersions(): Promise<VersionDTO[]> {
  ...
  const lastReleases: ReleaseDTO[] = releases
    .sort((a: ReleaseDTO, b: ReleaseDTO) => b.publishedAt.getTime() - a.publishedAt.getTime())
    .reduce((acc: ReleaseDTO[], release: ReleaseDTO) => {
      const existingRelease = acc.find(
        (existingRelease) => existingRelease.projectName === release.projectName && existingRelease.geo === release.geo
      )
      if (!existingRelease) {
        acc.push(release)
      }
