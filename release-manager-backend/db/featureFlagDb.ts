...
async function getFeatureFlagLogs(): Promise<FeatureFlagLog[]> {
  ...
  return (await dbClient.db.collection('featureFlagLogs').find().toArray()).map((res: any) => {
...
async function addFeatureFlagLog(featureFlagLog: FeatureFlagLog): Promise<void> {
  const dbClient = await DBClient.getClient()
